...
function Success() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      localStorage.removeItem("paymentSuccess"); // Clear after showing
    }
  }, []);

  if (!info) {
    return (
      <div className="pt-44 min-h-screen flex items-center justify-center bg-gray-100">
  ...
  return (
    <div className="pt-44 min-h-screen bg-gray-100">
      <div className="shadow-2xl shadow-gray-700 w-96 bg-white rounded-xl m-auto flex flex-col items-center p-6 space-y-4">
        <img
      ...
      <div className="w-96 bg-white rounded-xl shadow-md m-auto mt-6 p-6 space-y-3 text-gray-800">
        ...
        <h1 className="text-lg font-medium">
          Transaction Code:{" "}
          <span className="font-mono text-gray-700">{info.transaction_code}</span>
        </h1>
      </div>
    </div>
  );
